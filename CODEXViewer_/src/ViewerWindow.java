 ...
 * Modified according to the needs of CODEX Viewer 
*/
...
public class ViewerWindow extends StackWindow implements KeyListener {
    ...
    public ViewerWindow(ImagePlus imp, ViewerCanvas ic) {
    	super(imp, ic);
    }
	 ...
	 * @param imp
	 */
	public ViewerWindow(Image5D imp) {
		this(imp, new ViewerCanvas(imp));
	}
	 ...
	 * @param ic
	 */
	public ViewerWindow(Image5D imp, ViewerCanvas ic) {
		...
		setLayout(new ViewerLayout(ic));
        
        displayMode = ChannelControl.ONE_CHANNEL_COLOR;
		
		// Add ImageCanvas
		add(ic, ViewerLayout.CANVAS);	
       
        // Fill channelCanvasses vector
        for (int i=1; i<=i5d.getNChannels(); i++) {
            channelCanvasses.add(new ViewerCanvas(i5d.getChannelImagePlus(i)));
     ...
     */        
        ...
        for (int i=0; i<i5d.getNChannels(); i++) {
            ((ViewerCanvas)channelCanvasses.get(i)).removeKeyListener(ij);
        }
        ...
        for (int i=0; i<i5d.getNChannels(); i++) {
            ((ViewerCanvas)channelCanvasses.get(i)).addKeyListener(this);
        }
        ...
        for (int i=0; i<i5d.getNChannels(); i++) {
            ((ViewerCanvas)channelCanvasses.get(i)).addKeyListener(ij);
        }
    ...
    public void setDisplayMode(int displayMode) {
        ...
        if (displayMode == ChannelControl.TILED
            ...
            for (int i=0; i<i5d.getNChannels(); i++) {
                add((ViewerCanvas)channelCanvasses.get(i), ViewerLayout.CANVAS);
            }
        } else if (displayMode != ChannelControl.TILED 
                && this.displayMode == ChannelControl.TILED) {
            for (int i=0; i<i5d.getNChannels(); i++) {
                remove((ViewerCanvas)channelCanvasses.get(i));
            }
        }
...
//    }
        ...
        if (e.getID() == KeyEvent.KEY_PRESSED && (imp instanceof Image5D)) {             
            ...
            if (i5d == WindowManager.getCurrentImage()) {
                ...
                if (code==KeyEvent.VK_NUMPAD1 || (code==KeyEvent.VK_PAGE_DOWN&&shiftPressed )) {
                    ...
                    adaptRois((ViewerCanvas)getCanvas());                    
                } 
            }                                    
        }
    }
    ...
    public void setImagesUpdated() {
        ...
        for (int i=0; i<channelCanvasses.size(); i++) {
            ((ViewerCanvas)channelCanvasses.get(i)).setImageUpdated();
        }
    }
    ...
    public void repaintCanvasses() {
        ...
        if (channelCanvasses==null) return;
        for (int i=0; i<channelCanvasses.size(); i++) {
            ((ViewerCanvas)channelCanvasses.get(i)).repaint();
        }
    }
    
    public void adaptCanvasses(ViewerCanvas i5dc) {
        ...
        if (ic!=i5dc) {
            ViewerCanvas tmpCanvas = ((ViewerCanvas)ic);
        ...
        for (int i=0; i<channelCanvasses.size(); i++) {
            ViewerCanvas tmpCanvas = ((ViewerCanvas)channelCanvasses.get(i));
            if (tmpCanvas!=i5dc) {
                tmpCanvas.setSrcRectI5d((Rectangle)srcRect.clone());
                tmpCanvas.setMagnification(mag);
                tmpCanvas.setDrawingSize(drawingSize.width, drawingSize.height);
    ...
    public void adaptRois(ViewerCanvas i5dc) {   
     ...
     * to all channel canvasses of this window except <code>i5dc</code>. 
     */    
    public void adaptMouse (ViewerCanvas i5dc) { 
     ...
     */
    public int getCanvasChannelNumber(ViewerCanvas i5dc) {
        if (i5dc == ic) {
            return 0;
        }
        for (int i=0; i<channelCanvasses.size(); i++) {
    ...
    public void setChannelAsCurrent(ViewerCanvas i5dc) {
	...
	protected void drawRectangles() {
        ...
        if (layout != null && layout instanceof ViewerLayout) {      
            ViewerLayout i5dLayout = (ViewerLayout)layout;
